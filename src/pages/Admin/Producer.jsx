...
function createData(id, fullname, phone, address) {
  return {
    id,
    fullname,
    phone,
...
export default function Producer() {
  ...
  React.useEffect(() => {
    const getProducrs = async () => {
      try {
        const res = await producerApi.getAll();
        rows = res;
        setLoading(true);
      } catch (error) {
        setLoading(false);
  ...
  const handleClick = (event, row) => {
    ...
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, row.fullname);
  ...
  const handleView = (row) => {
    dispatch(setEditProducerModal({ status: true, data: row }));
  };
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <Box>
        <Fab
          ...
          onClick={() => dispatch(setAddProducerModal(true))}
        >
          <AddIcon />
        </Fab>
      </Box>
